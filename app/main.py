...
C411_URL = os.environ.get("C411_URL", "https://c411.org/api")
C411_API_KEY = os.environ.get("C411_API_KEY", "")
TR4KER_URL = os.environ.get("TR4KER_URL", "https://tr4ker.net/api")
TR4KER_API_KEY = os.environ.get("TR4KER_API_KEY", "")
V3X_URL = os.environ.get("V3X_URL", "https://api.v3x.club/torznab/api")
...
async def api_setup_fields():
    ...
    fields += [
        {"key": ck, "required": False, "secret": False, "label": label, "hint": hint, "recommended": rec}
        for ck, _real_key, label, hint, rec in settings_store.SINGLE_MANIFEST_FIELDS
    ]
...
async def api_setup_current(_: None = Depends(_require_admin)):
    ...
    for single_key, real_key, *_rest in settings_store.SINGLE_MANIFEST_FIELDS:
        status[single_key] = bool(os.environ.get(real_key))
    return status
...
async def api_setup_save(payload: dict, authorization: str = Header(default="")):
    ...
    for single_key, real_key, *_rest in settings_store.SINGLE_MANIFEST_FIELDS:
        raw = fields.pop(single_key, None)
        if raw:
            parsed_id = settings_store.parse_lumio_manifest_url(raw)
            if parsed_id is None:
                raise HTTPException(status_code=400, detail=f"Manifest invalide ({single_key})")
            fields[real_key] = parsed_id
    if SETUP_NEEDED:
